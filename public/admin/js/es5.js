...
var _loop = function _loop(_i) {
    oli.eq(_i).hover(function () {
        oli.eq(_i).css({ background: "#58A301" }).children("a").css({ color: "#fff" }).next().css({ display: "block" }).parent().siblings().css({ background: "#fff" }).children("a").css({ color: "#58A301" }).next().css({ display: "none" });
    }, function () {
        oli.eq(_i).css({ background: "#fff" }).children("a").css({ color: "#58A301" }).next().css({ display: "none" });
    });
};

for (var _i = 0; _i < oli.length; _i++) {
    _loop(_i);
}
//  充值
var oiput = $(".banner-right-1-bottom-2").children("p").children("input").length;
for (var _i2 = 0; _i2 < oiput; _i2++) {
    $(".banner-right-1-bottom-2").children("p").children("input").eq(_i2).click(function () {
...
for (var i = 0; i < json.length; i++) {
    arr += '<div class="main-2-2-connet-1" ip="' + json[i].IP + '"><div class="main-2-2-connet-1-2"><a href="#"> <img src="' + json[i].src + '"/><h3 class="go">' + json[i].haha + '</h3></a><p><i>' + json[i].name + '</i></p><p><span>' + json[i].price + '</span><b>' + json[i].price1 + '</b></p></div></div>';
}
